...
public class LapPhieuThueGUI extends JDialog implements ActionListener, MouseListener {
	...
	public LapPhieuThueGUI(Frame parent) {
		...
		tableDSDia.setModel(tmDia = new DefaultTableModel(new Object[][] {},
				new String[] { "STT", "Mã đĩa", "Trạng thái", "Số ngày thuê", "Tên tựa", "Mô tả", "Phí trễ hạn", "Giá thuê"}) {
			/**
					 * 
					 */
					private static final long serialVersionUID = 1L;
		...
		}else if(e.getSource().equals(btnThueDia))
		{
			for(int count = 0; count < tmDia.getRowCount(); count++)
			{
				if(txtMaKhachHang_1.getText().toString().equals(""))
				{
					...
					NhanVien nv = new NhanVien("NV00011","Nguyen Hai Phong", "021434324", "123", "");
	...
	private void loadDiaVaoBang() {
		if(tmDia.getRowCount()<0)
		{
			String[] tx = {
					...
					dia.getMaDia(),
					dia.getTrangThai(),
					String.valueOf(dia.getTuaDia().getSoNgayThue()),
					dia.getTuaDia().getTenTua(),
					dia.getTuaDia().getMoTa(),
					String.valueOf(dia.getTuaDia().getPhiTreHan()),
